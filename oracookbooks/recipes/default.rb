...
execute 'move-oracle-binary' do
  command "mv /vagrant/linuxamd64_12102_database_* #{node[:oracle][:database][:install_dir]}/"
...
if node[:oracle][:install_oracle_software]
  ...
  execute 'create_database' do
    command "su #{node[:oracle][:user][:name]} -l -c '#{node[:oracle][:database][:home]}/bin/dbca -silent -responseFile #{node[:oracle][:database][:install_dir]}/database/response/dbca.rsp'"
  end
end
